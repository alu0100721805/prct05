   ...
   class Fraccion
	 ...
	 include Comparable
	   ...
	   def simplificar()
	    ...
	    # El siguiente método se implementa para que la clase sepa compararse con una instacia de su misma clase
	   def <=> (fraccion)
		
	       raise ArgumentError,'Argumento "fraccion" no debe ser "nulo"' unless fraccion != nil
	       raise ArgumentError,'Argumento "fraccion" no es del tipo Amatista::Fraccion"' unless fraccion.is_a? Amatista::Fraccion
	        (self.numerador/self.denominador) <=> (fraccion.numerador/fraccion.denominador)
	   end	  
